...
window.dom = {
  create: function create(string) {
    //用于创建节点
    var container = document.createElement //创建一个元素
  ...
  after: function after(node, node2) {
    //用于新增弟弟，node2放到node的后面
    node.parentNode.insertBefore(node2, node.nextSibling); //先找到node的爸爸，把node2查到node的下一个节点的前面
  },
  ...
  remove: function remove(node) {
    //用于删除节点
    node.parentNode.removeChild(node); //让这个节点的上级节点删除子节点
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "9066" + '/');
